...
def main():
    ...
    logger.info(Message(title = f"Start News Trade - Time: {datetime.fromtimestamp(int(time.time()), tz=pytz.timezone('America/Chicago'))}", body=f"{json.dumps(config.beautify(), indent=2)}"), True)
