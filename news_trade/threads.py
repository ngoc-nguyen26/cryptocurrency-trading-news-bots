...
class Threads:
    ...
    def scrape_profile(self, username: str) -> dict:
        ...
        if process.returncode != 0:
            ...
            msg = msg.replace('\u25a0', '')
            self.logger.info(Message(title=f'Playwright installation successful - Time: {datetime.fromtimestamp(int(time.time()), tz=pytz.timezone('America/Chicago'))}', body=msg), True)
